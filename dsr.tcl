...
proc finish {} {
		...
		$ns_ flush-trace
		close $namtrace 
		exec xgraph  EMAP_3.xg proposed_3.xg -geometry 800x400 -t " PERFORMACE ANALYSIS ON delay time" -x "No of messages" -y "authentication delay time" -bg white & 
        exec xgraph  proposed-MDG1.xg existing-MDG1.xg   -geometry 800x400 -t "MESSAGE DIGEST GENERATION TIME TAKEN ANALYSIS" -x "key-size" -y "TIME-TAKEN(MS)" -bg white & 
    	exec xgraph proposed.xg existing.xg -t "Throughput" -x "Time" -y "Throughput" &
	exec xgraph  proposed-key-generation1.xg  existing-key-generation1.xg   -geometry 800x400 -t "KEY GENERATION TIME TAKEN ANALYSIS" -x "key-size(kb)" -y "TIME-TAKEN(MS)" -bg white & 
